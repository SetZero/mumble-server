...
static PROVIDERS: LazyLock<Vec<Provider>> = LazyLock::new(|| {
    vec![
        Provider::new(
            "YouTube",
            r"(?i)https?://(?:www\.)?youtube\.com/(?:watch|shorts/)",
        ...
        Provider::new(
            "Twitter/X",
            r"(?i)https?://(?:www\.)?(twitter|x)\.com/.+/status/",
            "https://publish.twitter.com/oembed",
        ),
        Provider::new(
            "Spotify",
            r"(?i)https?://open\.spotify\.com/",
            "https://open.spotify.com/oembed",
        ),
        Provider::new(
            "SoundCloud",
            r"(?i)https?://soundcloud\.com/",
            "https://soundcloud.com/oembed",
        ),
        Provider::new(
            "Twitch",
            r"(?i)https?://(?:www|clips)\.twitch\.tv/",
            "https://api.twitch.tv/v5/oembed",
        ),
        Provider::new(
            "TikTok",
            r"(?i)https?://(?:www\.)?tiktok\.com/",
            "https://www.tiktok.com/oembed",
        ),
        Provider::new(
            "Reddit",
            r"(?i)https?://(?:www\.)?reddit\.com/r/",
...
pub async fn fetch_preview(
    ...
    let text =
        match fetch_text_capped(client, &endpoint, "application/json", MAX_RESPONSE_BYTES).await {
            Ok(t) => t,
            Err(e) => {
...
#[allow(
    clippy::expect_used,
    reason = "hardcoded regex literal, cannot fail to compile"
)]
...
mod tests {
    ...
    fn matches_known_providers() {
        assert_eq!(
            match_provider(&Url::parse("https://www.youtube.com/watch?v=abc").unwrap())
                .map(|p| p.name),
            Some("YouTube")
        );
        assert_eq!(
    ...
    fn reddit_og_transform() {
        let out =
            transform_for_og(&Url::parse("https://www.reddit.com/r/rust/comments/1").unwrap());
