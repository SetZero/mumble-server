...
public:
	...
	/// TOTP shared secret (base32) generated by a TOTP_BEGIN account action
	/// but not yet confirmed via TOTP_VERIFY. Session-local only: it is
	/// persisted to the account exactly when the user proves possession by
	/// submitting a valid code, and is lost on disconnect.
	QString qsPendingTotpSecret;
